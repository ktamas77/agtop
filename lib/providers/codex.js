...
function queryThreads() {
  ...
  try {
    // No -readonly: it fails with SQLITE_CANTOPEN on Codex's WAL-mode DB.
    // stderr is ignored so a failed open never leaks into the TUI.
...
// Codex's configured default model from ~/.codex/config.toml — used as the
// model fallback for live sessions that haven't been persisted to a thread yet.
function configModel() {
  try {
    const toml = fs.readFileSync(path.join(os.homedir(), '.codex', 'config.toml'), 'utf8');
    // Top-level `model = "..."` (before any [section]); ignore per-profile keys.
...
function collect(procs) {
  ...
  return procs.map((p) => {
    ...
    let model = cfgModel; // default; a matched thread overrides below
    let version = null;
    let lastTs = null;
    let lastPrompt = null;
    let sessionId = null;
